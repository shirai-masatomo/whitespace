...
レビュー基準はmainのd731a53232dae287fd07cbc45df1232edbcc64d2。追加124件を68採用・43保留・13除外とし、既存20件を含め88件が有効になった。追加採用の19件は既存語の部分一致で認識済み、単独で新たに認識できる候補は49件だった。「話してください」はlisteningへ訂正。ユーザー本人が全件を承認したという意味ではない。
採用分はgenerated。外部辞書由来17件は4保留・13除外。2026-09-03に利用条件を調査し、09-05の別環境での再取得は失敗した。参照版と配布時に必要な表示は再確認が必要で、ライセンス全文は同梱していない。元のレビュー時点の旧挙動は現行仕様ではない。
...
- 肯定同士は傾聴→謝罪→安心で代表を選ぶ。肯定と敵意・拒絶等が混在する場合は判断保留にする。
- 引用の伝聞形、二重否定、面倒を見たい、お前の話は限定的に保留。対応範囲と未対応例は [開発資料](DEVELOPMENT.md) を参照。
- 残る課題と次の作業は [現在地](STATUS.md) に集約する。
