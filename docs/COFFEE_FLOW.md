...
会話の自然さと操作の省略を議論するための代表図です。箱は現行実装の台詞・動作の抜粋または要約。全順列を列挙せず、意味が変わる分岐を示します。事情はタイトル下で指定でき、「事情を知らずに始める」から設定名を伏せた試遊もできます。
...
Aの友人。最初から見えるのは濡れた端だけで、大切さや背景はまだ知りません。「あなたの味方だ」は「ほかの言い方」に残しています。
...
flowchart TB
  ...
  pending --> entrust
  pending --> paper
  request --> entrust["移すのもお願いできる？"]
  request --> paper
  move --> player["〈あなたがノートを移す〉<br/>相手：ありがとう。あとは紙で押さえてみる。"]
  entrust --> owner["〈相手が自分でノートを移す〉<br/>相手：うん、じゃあ自分で移すね。"]
```

移動だけでは水気は取れません。どちらからも紙を差し出す経路へ続けます。「分かった」は了承のみ。「移すのもお願いできる？」は作業を本人へ委ねる応答で、主人公が実行したことにはしません。了承の繰り返しでも物は動きません。
...
通常画面では了解の返答を会話開始後に出し、ノート移動は物が見えるときだけ表示します。「味方だ」や事故に関与していない側の「ごめん」は折りたたみへ整理し、不適切になり得る発言も選べるよう残しています。検証欄は全13操作を扱います。
...
- 了承と委任を分けた。「移すのもお願いできる？」が意図を伝えるか、初見の人に確かめたい。
