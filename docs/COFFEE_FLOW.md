...
```mermaid
flowchart TB
  start["友人の席へ来た<br/>机にコーヒーがこぼれている"] --> offer["あなた：ティッシュいる？"]
  offer --> yes["相手：うん、ティッシュがほしい。<br/>拭くのは自分でできるから。"]
  yes --> give["ティッシュを渡す"]
  yes --> space["そっとしておく"]
  yes --> watch["様子を見る"]
  give --> received["〈相手が受け取る〉<br/>相手：ありがとう、助かる。"]
  received --> wait["拭くのを見守る"]
  wait --> clean["〈相手が机を拭く〉<br/>相手：これで大丈夫。ありがとう。"]
  space --> away["〈あなたが一歩下がる〉<br/>相手：少し落ち着きたい。<br/>受取同意はいったん解除"]
  away --> again["再び：ティッシュいる？"]
  again -->|"そばに戻り、同意し直す"| yes
  watch --> wet["〈机にはコーヒーが溜まったまま〉<br/>相手はまだ拭かない"]
  wet -->|"同じ選択地点へ"| yes
```

「様子を見る」だけでは物は動きません。紙を持つ相手に「拭くのを見守る」を選ぶと拭きます。受取後の「渡す」は無効。拭いた後も会話は続けられます。上の戻り矢印は同じ選択地点への復帰で、過去の知識・謝罪・拒否の記憶を消す意味ではありません。
