...
## 現在のマイルストーン: intent候補のレビューと採用後の確認

- `docs/INTENT_DICTIONARY_REVIEW.md` のレビュー待ち候補を確認する。
- 特に `confidence: medium` と `low` の候補を優先して確認する。
- 各候補を採用・保留・除外のいずれかに判断する。
- 採用する表現だけをintent単位で確認して `status: adopted` に変更し、理由を記録する。
- 保留は `deferred`、除外は `excluded` とし、未レビューの `review` と区別する。
- 変更後に自動テスト、lint、production build、ブラウザ確認が成功する。
- ゲームバランスの変更は、辞書レビューとは別の作業として扱う。

現在の到達状況（2026-09-05）: ユーザーから委任されたassistantのレビューと採否反映、テスト10件、lint、buildは済み。ブラウザがローカルURLを拒否したため、**ブラウザ確認条件は未達であり、マイルストーン全体は未完了**。確認手順は [レビュー結果](docs/INTENT_REVIEW_OUTCOME.md#未確認のブラウザ手順) を使う。
