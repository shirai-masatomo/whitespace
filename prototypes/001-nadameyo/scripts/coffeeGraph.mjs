...
export function exploreCoffee(condition, inspect = () => {}) {
  ...
  let transitions = 0, selfLoops = 0, minDry = Infinity, dryWays = 0
  ...
  for (let i = 0; i < queue.length; i++) {
    ...
    for (const choice of CHOICES) {
      ...
      if (previousIndex === undefined) {
        if (queue.length > 100000) throw new Error('Finite-state bound exceeded')
  ...
  return { condition, states: queue.length, transitions, selfLoops, minDry, dryWays, phases: [...phases], branches, phaseEdges, nodes: queue.map(sceneSnapshot), edges }
}
