...
const ambiguityGuards = [
  [/大丈夫(?:じゃ|では|でも)ない|大丈夫とは(?:思わない|言えない)/, '安心を否定する表現のため判断を保留'],
...
export function createIntentEvaluator(dictionary) {
  ...
  return function evaluate({ input }) {
    ...
    for (const entry of expressions) {
      ...
      while (start !== -1) {
        matches.push({ text: entry.text, intent: entry.intent, status: entry.status,
          start, end: start + entry.normalized.length })
    ...
    const result = (disposition, reason, intent = 'unknown') => ({
      intent, disposition, reason, normalizedInput, matches: effective, engine: 'dictionary-v2',
    })
