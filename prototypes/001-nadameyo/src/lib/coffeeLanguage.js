...
export const COFFEE_PROMPT_VERSION = 'coffee-v6'
...
1回に1行為だけ対応する。別れを告げて会話を終える（またね/では失礼します/ゆっくり休んでね）はleave。一時的にそっとしておく行動はgive_space。援助を引き受けない「手を貸せない」はdecline_help。これは了承でも委任でもない。提案(いる？/渡そうか)はoffer_tissue。実際に渡す・差し出す明示はgive_tissue（受け取るかは相手が決める）。了解・分かったはacknowledgeで、依頼の了承だけ。作業を相手へ委ねる「任せる」「ノートはそっちで移してもらえる？」はentrust。直前に依頼されても、了解だけをentrustにしない。否定された行為を肯定に変換しない。
