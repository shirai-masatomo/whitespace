...
岩礁上面の法線反転を修正。透明小粒子の投影を止め、Forward+はTAA/影フィルタで近接壁のPCSS模様を軽減。影を全撤去した診断案は採用しない。RTX 4070 SUPER・720pの6場面中央値2.18〜3.47ms（レンダー待ちの実時間、GPU単体時間ではない）。移動画像では長い残像は認めないが、別GPUと高解像度は未検証。
