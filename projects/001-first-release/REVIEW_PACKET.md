...
- 全補給を泡と発光する葉を持つ酸素藻へ。葉先まで接触回復の範囲に収めた。
- 140/225mの補給迂回路を追加。直行と安全寄りで異なる時間/酸素余裕。
- 複数の目標候補を表示。魚群と潮流の粒子で海の動きも追加。
- 新しい迂回地点で岸壁へカメラが入る問題を修正。実メッシュで遮蔽を検査。
...
成功: format/lint、依存整合性、ルール404・シーン174、6経路と対照条件、Windows export/headless起動、実GPU自動プレイ。Actions証跡は[STATE](PROJECT_STATE.md)。CIはGPU画質を検証しない。配布EXEの通常GPU起動と、ソース側の操作付き画面検証を区別する。
...
[測定JSON](review/evaluation.json)。急降下直行は33.37秒・最低酸素30.83%、同じ急降下操縦で補給迂回は47.60秒・52.63%。潮流を通る経路は39.80秒、同じ操作の無潮流対照40.38秒。最速の全探索ではなく、比較した経路間の差。

自動操縦は理想的に目標へ進むため、人間の迷い・失敗率・面白さを保証しない。6経路合計で通常視点の目標35/43、主人公43/43、F43/43。岸壁を含む遮蔽も確認。海上出発と一部の寄り道は改善対象。
...
| --- | --- | --- |
| 中央値（3場面） | 2.13 / 2.06 / 2.04ms | 0.89 / 1.00 / 0.90ms |
| P95最大 | 3.98ms | 1.12ms |
...
**動くからPASSにはしない。** 製品として見せるVertical Sliceの画質ゲートは未達。DD-036/037は反映。DD-035（接地/視線）→DD-034（海面/遠景）→DD-027（品質ゲート）を人間待ちなしで継続。
