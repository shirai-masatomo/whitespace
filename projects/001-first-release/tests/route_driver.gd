...
static func input_for(model, target_index: int) -> Vector3:
...
static func reach(model, target_index: int, timeout: float = 40.0) -> bool:
	for frame in range(int(timeout * 60)):
		var command := input_for(model, target_index)
