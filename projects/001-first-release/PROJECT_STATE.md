...
- `check -BuildFolder windows-real`: format/lint/依存整合性、ルール396・シーン397・音52、6経路と対照条件、Windows Release export/EXE headless起動。
...
- RTX 4070 SUPER、1280×720、VSyncなし、静止3場面: Forward+中央値2.04〜2.37ms、P95最大4.09ms。GL中央値0.94〜1.08ms。単体GPU時間ではなく描画フレームの実時間。別GPUは未確認。
- 直前CI: [Windows Actions #35433135401](https://github.com/shirai-masatomo/whitespace/actions/runs/35433135401) 成功、main `b1680e7`（酸素藻・海面・接地・PC非干渉）。以後の救助/音修正はローカルcheck・GPU確認済み、次commitでCI再確認。
- [画面・連続画像・評価](REVIEW_PACKET.md) / [自己レビュー](AI_REVIEW.md)。自動操縦の成功を面白さや製品品質の証明にしない。

## 次

[人間方針](HUMAN_DIRECTION.md)の酸素藻と複数ルートを反映。13足場/7補給群落、泡/発光する葉と魚群。急降下直行33.08秒・最低酸素32.40%、同じ急降下で補給迂回47.30秒・52.63%。潮流は無潮流対照より0.58秒速い。流木の接地と海上カメラ（DD-035）、海面/遠景（DD-034）は検証済み。PC非干渉と補給中の見通しも両rendererで確認。操作中断91ケースを測定し、長い救助を最大5.25→3.48秒へ短縮。環境/操作音も追加。次は実メッシュで見つかった岩の縁の接地差をDD-041で修正し、DD-027品質監査へ。環境音/操作音は実装、保存/Steamは未実装。
