...
深度損失の測定条件: 60m補給地点を登録し、足場のない水中で満タンから自然沈降 / E / Qを維持すると、それぞれ124.08 / 221.84 / 49.88mを失う。90mの普通の足場で待つ場合は30m。実際の損失はルートと失敗位置によって変わる。代表画像の失敗は90m→60mで30m損失。全足場をたどる自動操作は104.03秒、失敗0回でクリア。

### カメラ比較と今の問題点

同じ位置・角度で比較: [三人称](review/03-platform.png) / [一人称](review/camera-first-person.png)。三人称は足元・足場の縁・下の複数足場を見渡せるため標準に採用。一人称は海への没入感がある一方、足場中央から下を見ると床が画面を占め、着地位置も分かりにくい。Vで随時比較可能。三人称カメラは足場との遮蔽をレイ判定で避ける。

- 足場はまだ単純で広く、経路の選択肢と難易度は未調整。普通の足場を飛ばすルートも許す。
- 床に隠れる次の足場は深度付きマーカーで補助。視点によって目印が画面外になるため、誘導の分かりやすさは人間レビューが必要。
...
- `check`: format / lint / 開発依存整合性 / import / ルール・シーンテスト / Windows release build / 出力EXEのheadless起動 / 配布zip。
- `visual`: GPUで8状態・カメラ比較・浮上後のゴール・再プレイ・960×540のポーズを検証。`artifacts/*.png` を目視確認し、レビュー用8枚と一人称比較を `review/` へ更新する。
...
- [PROJECT_STATE](PROJECT_STATE.md) / [TASKS](TASKS.md) / [AUTONOMY](AUTONOMY.md): 新しいセッションで最初に読む。
