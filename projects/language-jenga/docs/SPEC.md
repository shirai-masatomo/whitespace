...
このファイルは、言語ジェンガ（仮）で「何を作るか」「現在の実装がどのような仕様か」を記録する。

## プロジェクトの目的

言葉と関係性の変化をゲームにするため、小さな会話実験を重ねる。WhiteSpace全体の制作方針は親リポジトリのREADMEで管理する。
...
ゲームで使うintent定義と表現データは `app/nadameyo/src/data/intent-dictionary.json`、判定処理は `app/nadameyo/src/lib/intentMatcher.js` に置く。
...
- `PROJECT_STATE.md`: 言語ジェンガの現在地を記録する。
- `docs/SPEC.md`: プロジェクトとアプリの仕様を記録する。
- `docs/ACCEPTANCE.md`: マイルストーンの完了条件を記録する。
- `docs/TODO.md`: 次に実行する小さな作業を管理する。
- `docs/COMMAND_LOG.md`: 重要なコマンドの実行履歴を記録する。
- `docs/INTENT_DICTIONARY_REVIEW.md`: intent候補の採否をレビューする。
- `app/nadameyo/APP_STATE.md`: 「宥めよ」固有の状態、仕様、変更履歴を記録する。
- `README.md`: プロジェクト構成と各文書の役割を案内する。
